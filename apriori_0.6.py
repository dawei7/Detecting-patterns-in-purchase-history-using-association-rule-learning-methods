...
def apriori(transactions_ungrouped, min_support_perc=0.5, min_confidence=0.5,min_lift=1,min_associations=2,max_associations=5):
    ...
    def get_combinations(old_combinations):
        ...
        if counter == 1:
            ...
            rights = list(set(list(itertools.chain(*[v for k,v in combinations.items() if len(k)==counter])))) #list(set().union(*old_combinations.keys()))
...
df_apriori = apriori(transactions, min_support_perc=0.005, min_confidence=0.5, min_lift=0.5,min_associations=3,max_associations=3)
