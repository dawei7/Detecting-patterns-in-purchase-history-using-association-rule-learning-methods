...
def associationRule(assoc_rules, headerTable, minConf, numTransaction):
...
def fpgrowthFromDataFrame(data, minSupRatio=0.001, maxSupRatio=1, minConf=0, item_col=1, date_col=False, profit_col=False, max_date = False, date_range=False, date_sensitivity = lambda x: 1 / (1 + math.exp(-10*x+5)), max_profit = False, profit_sensitivity = lambda x : 1*x):
    ...
    if(fpTree == None):
        ...
        rules = associationRule(assoc_rules, headerTable, minConf, numTransaction)
...
"""
data = pd.read_csv("kz_part_1 copy 2.csv") #Smaller Subset of Original Kaggle DataSet for Testing
...
data = pd.read_csv("transactions_V3.csv")
data = data.groupby("transaction",dropna=True)["item"].agg([lambda x: list(x),"count"])

# rules = fpgrowthFromDataFrame(data, minSupRatio=0.001, maxSupRatio=1, minConf=0, item_col=1, date_col=2, profit_col=3, max_date=max_date, date_range=90, date_sensitivity = lambda x: 1 / (1 + math.exp(-10*x+5)), max_profit = max_profit, profit_sensitivity = lambda x : 1 * x) #Complete

# rules = fpgrowthFromDataFrame(data, minSupRatio=0.001, maxSupRatio=1, minConf=0, item_col=1, date_col=2, max_date=max_date, date_range=90, date_sensitivity = lambda x: 1 / (1 + math.exp(-10*x+5))) #Only Date

# rules = fpgrowthFromDataFrame(data, minSupRatio=0.001, maxSupRatio=1, minConf=0, item_col=1, profit_col=3, max_profit = max_profit, profit_sensitivity = lambda x : 1 * x) #Only Profit

rules = fpgrowthFromDataFrame(data, minSupRatio=0.001, maxSupRatio=1, minConf=0, item_col=1) #Traditional
