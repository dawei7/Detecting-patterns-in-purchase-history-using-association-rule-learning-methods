...
def associationRule(assoc_rules, headerTable, minConf, minSup, numTransaction):
    ...
    for item in headerTable_keys:
        ...
        for my_combination in my_combinations:
            ...
            if support >= minSup and support != 0:
...
data_simple = data_simple.groupby("transaction",dropna=True)["item"].agg([lambda x: list(x),"count"])

rules = fpgrowthFromDataFrame(data_simple, minSupRatio=0.6, maxSupRatio=1, minConf=0, item_col=1) #Traditional Association Rules
